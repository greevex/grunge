    ...
    public function connect()
    {
        ...
        for($attempt = 5; $attempt > 0 && !$connected; $attempt--) {
            try {
                ...
                if(!$this->sock) {
                    debug::put("Error Creating Socket: " . socket_strerror(socket_last_error()), __METHOD__);
                }
                socket_set_option($this->sock, SOL_SOCKET, SO_RCVTIMEO, array("sec" => 1, "usec" => 0));
                socket_connect($this->sock, $this->host, $this->port);
                if(!$this->sock) {
                    debug::put("Unable to connect to Socket: " . socket_strerror(socket_last_error()), __METHOD__);
                }
                $connected = true;
            } catch(\Exception $e) {
                debug::put("Error! Code: {$e->getCode()} / Message: {$e->getMessage()} in {$e->getFile()}:{$e->getLine()}", __METHOD__);
            }
        }
        debug::put("Client [{$this->name}] created![{$attempt}]", __METHOD__);
